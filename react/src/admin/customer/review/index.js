...
function ListReview() {
  ...
  const [notification, setNotification] = useState({
    ...
    const { data: reviewsData, isLoading, error } = useQuery({
      queryKey: ['reviews'],
      queryFn: () => CrudService.getReviews(),
      onError: (error) => {
  ...
  const clickViewHandler = (id) => {
    navigate(`/review/detail-review/${id}`);
  };
  ...
  const clickDeleteHandler = async (e, id) => {
    try {
      if (!confirm("Are you sure you want to delete this review?")) {
        ...
        await CrudService.deleteReview(id);
        const response = await CrudService.getReviews();
    ...
    columns: [
...

      {
        ...
        Cell: ({ cell: { value } }) => (
          ...
          linkPath={`/review/detail-review/${value.id}`}
          />
        ),
      },
      ...
      { Header: "Rating", accessor: "rating", Cell: ({ row, value }) => (
        <div onClick={() => clickViewHandler(row.original.id)} style={{ cursor: "pointer" }}>
          {value}
        </div>
      )},
      ...
      { Header: "Product", accessor: "product", Cell: ({ row, value }) => (
        <div onClick={() => clickViewHandler(row.original.id)} style={{ cursor: "pointer" }}>
          {value}
        </div>
      )},      
  ...
  return (
    <DashboardLayout>
      ...
      <ListActionHeader title="New Review" clickAddHandler={clickAddHandler} />
