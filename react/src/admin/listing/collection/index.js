...
function ListCollection() {
  ...
  const handleEditRow = useCallback((id) => navigate(`/collection/detail-collection/${id}`), [navigate]);


  const handleViewRow = useCallback((id) => navigate(`/collection/detail-collection/${id}`), [navigate]);
  ...
  const columns = [
    
    {
      ...
      hideable: false,
      renderCell: (params) => (
        <RenderCellCollection params={params} onViewRow={() => handleViewRow(params.row.listingId)} />
      ),
    },
