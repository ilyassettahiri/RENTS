...
function ListCheckingOut() {
  ...
  const [notification, setNotification] = useState({
    ...
    const { data: listingsData, isLoading, error } = useQuery({
      queryKey: ['checkingOuts'],
      queryFn: () => CrudService.getCheckingOuts(),
      onError: (error) => {
        console.error('Failed to fetch checking outs:', error);
      },
    });
  ...
  const handleEditRow = useCallback((id) => navigate(`/reservation/edit-reservation/${id}`), [navigate]);


  const handleViewRow = useCallback((id) => navigate(`/reservation/detail-reservation/${id}`), [navigate]);
  ...
  const columns = [
...

    {
      ...
      hideable: false,
      renderCell: (params) => (
        <RenderCellCustomer params={params} onViewRow={() => handleViewRow(params.row.listingId)} />
      ),
    },
